...
const SearchModal = () => {
 ...
 const Map = useMemo(() => dynamic(() => import('../Map'), { ssr: false }), [location])
    ...
    if(step === STEPS.INFO) {
        bodyContent = (
            <div className="flex flex-col gap-8">
              ...
              value={guestCount} onChange={(value) => setGuestCount(value)}/>

              <Counter title="Rooms" subtitle="How many rooms do you need?"
              value={roomCount} onChange={(value) => setRoomCount(value)}/>

              <Counter title="Bathrooms" subtitle="How many bathrooms do you need?"
              value={bathroomCount} onChange={(value) => setBathroomCount(value)}/>
            </div>
        )
    }
  ...
  return (
    ...
    title='Filters' actionLabel={actionLabel} body={bodyContent}
     secondaryAction={step === STEPS.LOCATION ? undefined : onBack}
      secondaryActionLabel={secondatyActionLabel}/>
  )
}
